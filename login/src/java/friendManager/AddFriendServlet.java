...
public class AddFriendServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        if(name.length()==0||phone.length()==0||email.length()==0||workPlace.length()==0||QQ.length()==0){
            wrong1();
            response.sendRedirect("http://localhost:8080/login/friendManager/addFriend.jsp");
        }else{
            try{
                Connection con=null;
                Statement stmt=null;
                ...
                if(rs.next()){
                    ...
                    String sql2="insert into friend"+"(sname,name,phone,email,workPlace,place,qq)"+"values("+"'"+userName+"'"+","+"'"+name+"'"+","+"'"+phone+"'"+","+"'"+email+"'"+","+"'"+workPlace+"'"+","+"'"+place+"'"+","+"'"+QQ+"'"+")";
