    ...
    throws ServletException, IOException {
        ...
        if(phone.length()==0||email.length()==0||workPlace.length()==0||place.length()==0||QQ.length()==0){
            ...
            try{
                ...
                String sql1="update friend set phone='"+phone+"',email='"+email+"',workPlace='"+workPlace+"',place='"+place+"',qq='"+QQ+"' where name='"+name+"'and sname='"+userName+"'";
                stmt.executeUpdate(sql1);
                String sql2="select * from friend where sname='"+userName+"'";
                ...
                con.close();
                right();
                response.sendRedirect("http://localhost:8080/login/LookFriendServlet");
            }catch(Exception e){
                e.printStackTrace();
            }
        }
