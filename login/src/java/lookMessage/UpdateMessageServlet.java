...
public class UpdateMessageServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        if(phone.length()==0||email.length()==0){
            wrong1();
            response.sendRedirect("http://localhost:8080/login/lookMessage/updateMessage.jsp");
        }else{
            try{
                Connection con=null;
                Statement stmt=null;
                ...
                String sql1="Update user set edu='"+edu+"',work='"+work+"',phone='"+phone+"',email='"+email+"' where sname='"+userName+"'";
