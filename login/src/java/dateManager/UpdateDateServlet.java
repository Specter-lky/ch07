...
public class UpdateDateServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        if(year.length()==0||month.length()==0||day.length()==0){
            wrong1();
            response.sendRedirect("http://localhost:8080/login/dateManager/updateDate.jsp");
            ...
            try{
                ...
                String sql1="select * from date where date='"+date+"'and sname='"+userName+"'";
                rs=stmt.executeQuery(sql1);
                if(rs.next()){
                    String sql2="update date set thing='"+thing+"' where date='"+date+"'and sname='"+userName+"'";
                    stmt.executeUpdate(sql2);
                    String sql3="select * from date where sname='"+userName+"'";
                    ...
                    con.close();
                    right();
                    response.sendRedirect("http://localhost:8080/login/dateManager/lookDate.jsp");
                }
                else{
                    wrong4();
                    response.sendRedirect("http://localhost:8080/login/dateManager/updateDate.jsp");
